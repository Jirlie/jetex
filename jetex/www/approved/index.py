...
def get_context(context):
    ...
    all = frappe.db.get_all("Laundry", filters={'employee': frappe.session.user, 'workflow_state': "Approved"})
    docs = []
    for all in all:
    ...
    all2 = frappe.db.get_all("Lost Item", filters={'employee': frappe.session.user, 'workflow_state': "Approved"})
    docs = []
    for all in all2:
    ...
    all3 = frappe.db.get_all("Damage Report", filters={'employee': frappe.session.user, 'workflow_state': "Approved"})
    docs = []
    for all in all3:
